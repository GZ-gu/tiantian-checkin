...
function renderModal() {
  ...
  if (modal.type === "checkin") {
    ...
    body = `${icon(`${A}/modal-lion.png?v=1.2.21`, "小狮子", "sheet-lion")}<h2>今天完成了吗？</h2><strong class="sheet-task">${escapeHtml(task.title)}</strong><div class="sheet-notice">${icon(`${A}/modal-success.svg`)}<span>确认后，${getCurrentMilestone(state, task).name}勋章将变为 ${getCurrentMilestone(state, task).segmentDone + 1}/${getCurrentMilestone(state, task).segmentTarget}</span></div><button class="primary-button" data-action="confirm-checkin" data-task-id="${task.id}" type="button">确认完成</button><button class="plain-button" data-action="close-modal" type="button">再等等</button>`;
  } else if (modal.type === "makeup" || modal.type === "revoke") {
    const makeup = modal.type === "makeup";
    body = `${icon(`${A}/modal-lion.png?v=1.2.21`, "小狮子", "sheet-lion")}<h2>${makeup ? "请说明补卡情况" : "请说明撤销情况"}</h2><form id="${makeup ? "makeupForm" : "revokeForm"}">${makeup && !modal.date ? `<div class="field"><label for="makeupDate">补卡日期</label><input class="field-control" id="makeupDate" name="date" type="date" max="${previousDate()}" required></div>` : makeup ? `<input type="hidden" name="date" value="${modal.date}"><p class="fixed-date">补卡日期：${formatDate(modal.date, true)}</p>` : ""}<div class="field"><textarea class="reason-control" id="reason" name="reason" maxlength="100" placeholder="请输入内容"></textarea><div class="field-meta"><span data-error="reason"></span><span data-count="reason">0/100</span></div></div><button class="primary-button" type="submit">确认完成</button><button class="plain-button" type="button" data-action="close-modal">再等等</button></form>`;
  } else if (modal.type === "confirm") {
    body = `${icon(`${A}/modal-lion.png?v=1.2.21`, "小狮子", "sheet-lion")}<h2>${escapeHtml(modal.title)}</h2><p class="confirm-message">${escapeHtml(modal.message)}</p><button class="primary-button" type="button" data-action="confirm-pending">确认</button><button class="plain-button" type="button" data-action="close-modal">取消</button>`;
  }
